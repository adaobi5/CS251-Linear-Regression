...
class Analysis:
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        '''
        dat = self.data.select_data([ind_var, dep_var])
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        '''
        dat = self.data.select_data(data_vars)

        fig,ax = plt.subplots(len(data_vars),len(data_vars), sharey = True, sharex = True, figsize=fig_sz)

        for i in range(len(data_vars)):
            for j in range(len(data_vars)):
                ax[i][j].scatter(dat[:,j],dat[:,i])
                # print(i, j)
